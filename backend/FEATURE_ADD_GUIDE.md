...
5. **Wire** — register in the relevant bootstrap module (for example `src/bootstrap/create-auth-module.ts` or `src/bootstrap/create-dashboard-module.ts`)
...
- Dashboard summary data is split into two endpoints: `GET /dashboard/summary` for the core dashboard payload and `GET /dashboard/summary/insights` for prediction/recommendation data
